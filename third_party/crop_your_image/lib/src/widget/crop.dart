...
typedef ScaleChangedCallback = void Function(double scale, double minScale);
...
class Crop extends StatelessWidget {
  ...
  /// Callback called when the current zoom scale changes.
  /// [minScale] represents the current minimum zoom that still covers the crop area.
  final ScaleChangedCallback? onScaleChanged;
  ...
  /// If provided, the heavy [imageParser] decode is deferred until [crop] is
  /// called. The widget uses this size for layout immediately, avoiding a
  /// multi-second freeze on Web debug (where [compute] cannot offload to a
  /// web worker).
  final Size? precomputedImageSize;
      ...
      builder: (c, constraints) {
        ...
        return MediaQuery(
          ...
          child: _CropEditor(
            ...
            precomputedImageSize: precomputedImageSize,
          ),
        );
      },
    );
...
class _CropEditorState extends State<_CropEditor> {
  ...
  /// Whether [_viewState] has been set to a [ReadyCropEditorViewState].
  bool _isReady = false;

  /// The [InitialRectBuilder] captured on the first [_resetCropRect] call,
  /// so that [CropController.reset] can replay it without depending on the
  /// (potentially stale) widget parameter.
  InitialRectBuilder? _initialRectBuilderCache;

  void _notifyScaleChanged() {
    if (!_viewState.isReady) {
      return;
    }
    widget.onScaleChanged?.call(_readyState.scale, _readyState.minimumScale);
  }
  ...
  void initState() {
    ...
    _cropController.delegate = CropControllerDelegate()
      ...
      ..onChangeScale = (request) {
        if (!_viewState.isReady) return;
        _historyState.pushHistory(_readyState);
        _applyScale(
          _readyState.scale + request.delta,
      ...
      ..onReset = (aspectRatio) {
        if (!_viewState.isReady) return;
        // Purge undo/redo so a stale state can't be resurrected.
        _historyState.clear();

        if (aspectRatio != null) {
          // Explicit ratio: "change preset" semantic — reset pan/zoom,
          // initialize crop rect at the memorized initial size with
          // the new ratio, and persist it in the view state.
          setState(() {
            _viewState = _readyState.resetCropRect().copyWith(
              aspectRatio: aspectRatio,
            );
          ...
          // No ratio override: "true reset" semantic — replay the
          // cached initial builder exactly as it ran on first setup.
          setState(() {
            _viewState = _readyState.resetCropRect();
            widget.onImageMoved?.call(_readyState.imageRect);
          });
          final builder =
              _initialRectBuilderCache ?? widget.initialRectBuilder;
          switch (builder) {
        ...
        if (widget.interactive) {
          _applyScale(_readyState.minimumScale);
        }
      };
  ...
  void didChangeDependencies() {
    final precomputed = widget.precomputedImageSize;
    if (precomputed != null) {
      // Fast path: use caller-supplied dimensions for layout, deferring the
      // heavy package:image decode to crop time.
      if (!_isReady) {
        final preparing = PreparingCropEditorViewState(
          viewportSize: MediaQuery.of(context).size,
          withCircleUi: widget.withCircleUi,
          aspectRatio: widget.aspectRatio,
        );
        _viewState = preparing.prepared(precomputed);
        _isReady = true;
        // Defer _resetCropRect to after the build phase — it fires
        // callbacks (onScaleChanged, onMoved) that may trigger
        // setState on an ancestor, which is illegal during build.
      ...
      // Default path: parse image to obtain dimensions (and cache the decoded
      // image for cropping).
      _parseImageWith(
        ...
        if (parsed != null) {
          if (_viewState is PreparingCropEditorViewState) {
            setState(() {
              _viewState =
                  (_viewState as PreparingCropEditorViewState).prepared(
  ...
  Future<void> _crop(bool withCircleShape) async {
    ...
    // Lazy parse: if precomputedImageSize was used, the heavy decode was
    // deferred. Run it now (inside the cropping spinner) without resetting
    // the view state (which _parseImageWith would do).
    if (_parsedImageDetail == null) {
      try {
        final format = widget.formatDetector?.call(widget.image);
        final parsed = await compute(
          _parseFunc,
          [widget.imageParser, format, widget.image],
        );
        if (!mounted) return;
        _parsedImageDetail = parsed;
        _detectedFormat = format;
      } catch (e, trace) {
        widget.onCropped(CropFailure(e, trace));
        widget.onStatusChanged?.call(CropStatus.ready);
        return;
      }
    }
